...
class ShippedOrderList extends Component
{
    ...
    public function OrderStatus()
    {
        ...
        } elseif ($this->status == 'delivered' || $this->status = 'cancelled' || $this->status == 'processing' || $this->status == 'returned') {
            //  Approve Order
            // dd($statusCheck);
            $order = Order::find($id);
    ...
    {
        $order = Order::whereStatus('shipped')->orderBy('id', 'DESC');
        ...
        return view('livewire.backend.order.shipped-order-list', [
            ...
            //Order::whereStatus('shipped')->orderBy('id', 'DESC')->get(),
        ]);
    }
}
