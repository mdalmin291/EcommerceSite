            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="sale_details_report_table" class="table table-striped table-bordered nowrap"
...
<script>
    $(document).ready(function(){
		...
		$('#sale_details_report_table').DataTable({
			processing: true,
			responsive: true,
			serverSide: true,
			ajax: {
				url: "{{ route('report.sale-details-report-data') }}",
				type: 'GET',
				cache: false,
				data : function ( d ) {
			...
			{ title: 'Customer Name', data: 'contact_name', name: 'contact_name' },
			...
			{ title: 'Sub Total', data: 'sale_subtotal', class: "sale_subtotal", name: 'sale_subtotal' },
			...
			footerCallback: function(row, data, start, end, display) {
				var api = this.api();
				$.each(['quantity', 'purchase_price', 'sale_subtotal'], function( index, value ) {
		...
		$(document).on('change','.updateTable',function () {
			$('#sale_details_report_table').DataTable().draw(true);
		});
	});
</script>
@endsection
