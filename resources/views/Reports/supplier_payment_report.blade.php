                        ...
                        <div class="col-sm-12">
                            <div class="search-box mr-2 mb-2 d-inline-block">
                                <div class="position-relative">
                                    <h4 class="card-title design_title">Supplier Payment Report</h4>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-3">
                            <div wire:ignore class="form-group">
                                ...
                                <select class="form-control form-select select2 updateTable" placeholder="Supplier"
            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="supplier_payment_report_table" class="table table-striped table-bordered nowrap"
...
<script>
    $(document).ready(function(){
		...
		$('#supplier_payment_report_table').DataTable({
			processing: true,
			responsive: true,
			serverSide: true,
			ajax: {
				url: "{{ route('transaction.customer-payment-report-data') }}",
				type: 'GET',
				cache: false,
				data : function ( d ) {
			...
			{ title: 'Supplier', data: 'contact_id', name: 'contact_id' },
			{ title: 'Payment Method', data: 'payment_method_id', name: 'payment_method_id' },
			{ title: 'Amount', data: 'total_amount', class: "total_amount", name: 'total_amount' }
			],
			...
			'csvHtml5',
            // 'excel',
			{extend: 'excelHtml5', title: '{{$companyInfo->name}} \n {{$companyInfo->address}} \n Supplier Payment Report',  footer:true,
				exportOptions:{
					columns: ":not(.not-show)"
				},
			},
			{extend: 'pdfHtml5', title: '{{$companyInfo->business_name}} \n {{$companyInfo->address}} \n Supplier Payment Report', orientation: 'landscape', pageSize: 'LEGAL', footer:true,
				exportOptions:{
					charset: "utf-8",
					columns: ":not(.not-show)"
				},
			...
			footerCallback: function(row, data, start, end, display) {
				var api = this.api();
				$.each(['total_amount'], function( index, value ) {
		...
		$(document).on('change','.updateTable',function () {
			$('#supplier_payment_report_table').DataTable().draw(true);
		});
	});
</script>
@endsection
