                        ...
                        <div class="col-sm-12">
                            <div class="search-box mr-2 mb-2 d-inline-block">
                                <div class="position-relative">
                                    <h4 class="card-title design_title">Sales Report</h4>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    $(document).ready(function(){
		...
		$('#sale_report_table').DataTable({
			...
			serverSide: true,
			ajax: {
				url: "{{ route('report.sale-report-data') }}",
				type: 'GET',
				cache: false,
				data : function ( d ) {
			...
			{ title: 'Date', data: 'sale_date', name: 'sale_date' },
			...
			{extend: 'pdfHtml5', title: '{{$companyInfo->business_name}} \n {{$companyInfo->address}} \n Sales Report', orientation: 'landscape', pageSize: 'LEGAL', footer:true,
				exportOptions:{
					charset: "utf-8",
					columns: ":not(.not-show)"
				},
