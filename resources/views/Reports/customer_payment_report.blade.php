                        ...
                        <div class="col-sm-12">
                            <div class="search-box mr-2 mb-2 d-inline-block">
                                <div class="position-relative">
                                    <h4 class="card-title design_title">Customer Payment Report</h4>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="customer_payment_report_table" class="table table-striped table-bordered nowrap"
...
<script>
    $(document).ready(function(){
		...
		$('#customer_payment_report_table').DataTable({
			processing: true,
			responsive: true,
			serverSide: true,
			ajax: {
			...
			{ title: 'Customer', data: 'contact_id', name: 'contact_id' },
			...
			'csvHtml5',
            // 'excel',
			{extend: 'excelHtml5', title: '{{$companyInfo->name}} \n {{$companyInfo->address}} \n Customer Payment Report',  footer:true,
				exportOptions:{
					columns: ":not(.not-show)"
				},
			},
			{extend: 'pdfHtml5', title: '{{$companyInfo->business_name}} \n {{$companyInfo->address}} \n Customer Payment Report', orientation: 'landscape', pageSize: 'LEGAL', footer:true,
				exportOptions:{
					charset: "utf-8",
					columns: ":not(.not-show)"
				},
		...
		$(document).on('change','.updateTable',function () {
			$('#customer_payment_report_table').DataTable().draw(true);
		});
	});
</script>
@endsection
