            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="purchase_ledger_reports_table" class="table table-striped table-bordered nowrap"
...
<script>
	$(document).ready(function(){
		...
		$('#purchase_ledger_reports_table').DataTable({
			processing: true,
			responsive: true,
			serverSide: true,
			ordering: false,
			...
			order: [[ 2, "desc" ]],
			ajax: {
				url: "{{ route('report.supplier-ledger-report-data') }}",
				type: 'GET',
				cache: false,
				data : function ( d ) {
			...
			'csvHtml5',
			{extend: 'excelHtml5',
            title: '{{$companyInfo->business_name}} \n {{$companyInfo->address}} \n Supplier Leadger Report', footer:true,
            messageTop: function() {
                return 'Supplier Name : '+$('#contact_id :selected').text()+' ';
            },
				exportOptions:{
		...
		$(document).on('change','.updateTable',function () {
			$('#purchase_ledger_reports_table').DataTable().draw(true);
		});
	});
</script>
@endsection
